...
#+begin_src emacs-lisp :results none :tangle yes
  (use-package org-roam
    :ensure t
    :custom
    (org-roam-directory "~/lab/emacs/RoamNotes")
...
We can also define capture template in a Org file under =~/lab/emacs/RoamNotes/Template= folder in a =BookNoteTemplate.org= Org file:
...
#+begin_src emacs-lisp :results none :tangle no
  ("b" "Book notes" plain
   (file "~/lab/emacs/RoamNotes/Template/BookNoteTemplate.org")
   :if-new (file+head "%<%Y%m%d%H%M%S>-${slug}.org" "#+title: ${title}\n")
   :unnarrowed t)
#+end_src

*** ONGOING Org Roam Dailies (Or Journal)
:LOGBOOK:
- State "ONGOING"    from              [2023-05-03 mer. 21:35] \\
  Finish the documentation. The default journal capture template is modified. We can also add mode capture template
:END:
#+begin_src emacs-lisp :results none :tangle no
(setq org-roam-dailies-directory "Journal/")
#+end_src

Benefit of using dailies is the ability to links notes.
Journals are considered as standard node.


** Key bindings
:PROPERTIES:
:ID:       c5bfae02-13c8-4344-8853-616496fd97a5
:END:
*** Org Roam
