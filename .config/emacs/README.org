...
- [[#mail][Mail]]
  ...
  - [[#mu4e-custom-actions-for-capturing][Mu4e Custom Actions for Capturing]]
...
** Mu4e Custom Actions for Capturing
We can set custom actions that allow to use org capture template without invoking the =org-capture= or =mu4e-org-store-and-capture= functions:
#+begin_src emacs-lisp :results none
