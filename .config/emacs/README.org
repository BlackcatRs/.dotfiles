...
- [[#org][Org]]
  ...
  - [[#code-blocks-evaluation][Code Blocks Evaluation]]
...
#+begin_src emacs-lisp :results none
  ...
  (hl-line-mode -1)           ; Highlight cursor pointing line
...
** Electric-pair-mode
Add automatically closing parenthesis, brackets etc:
...
#+begin_src emacs-lisp :results none
  ...
  ;; Sort by property in numeric asc order
  (defun vts/org-sort-by-property ()
    (interactive)
    (setq property
    ...
    (org-sort-entries nil ?f
                      (lambda ()
                        (string-to-number (or (org-entry-get nil property) "0"))
  		      ))
    )
  ...
  (use-package org
    ...
    :bind (
  	   ...
  	   ("C-c s" . vts/org-sort-by-property)
  	   ))
#+end_src
...
#+begin_src emacs-lisp :results none
  ...
  (setq org-capture-templates
        '(("c" "Unschedule Task" entry (file+headline voc/todo "Unscheduled Tasks")
          ...
          ("s" "Schedule Task" entry (file voc/events)
           "* TODO %?\nSCHEDULED: %^T\n%U" :empty-lines 1)

          ("d" "Deadline" entry (file+headline voc/events)
...
By default, Emacs enables only the evaluation of Emacs-lisp code blocks, but many other languages ​​are supported. To enable (or disable) other languages, you can customize the =org-babel-load-languages= variable.

Here is an example to enable calc and the Python language:
#+begin_src emacs-lisp :results none
...
#+begin_src emacs-lisp :results none
  (if (file-exists-p "~/.config/emacs/vars.el")
  	...
  	;; Ignore Org roam dalies from Org roam UI
  	(setq org-roam-db-node-include-function
  	      (lambda ()
  		(not (string-match-p "Journal/" (buffer-file-name)))))
  	)))
#+end_src
...
** Helper Functions
This is custom helper function to save all attachments in a mail : 
#+begin_src emacs-lisp :results none
  (defun vts/save-all-attachments ()
    "Save ALL attachments of current message to `mu4e-attachment-dir'
 without any prompt."
    (interactive)
    (let* ((parts (mu4e-view-mime-parts))
           (attachments
            (seq-filter (lambda (p) (plist-get p :attachment-like)) parts))
           (dir (expand-file-name mu4e-attachment-dir)))

      (unless attachments
        (user-error "No attachments in this message"))
      ...
      (seq-do
       (lambda (part)
         (let* ((fname (plist-get part :filename))
                (handle (plist-get part :handle))
                (target
                 (funcall mu4e-uniquify-save-file-name-function
                          (mu4e-join-paths dir fname))))
           (when handle
             (mm-save-part-to-file handle target)
             (message "Saved: %s" target))))
       attachments)))
#+end_src
...
Configuration for the Atomicl mail provider:
#+begin_src emacs-lisp :results none :tangle ./mails/mu4e-context-atomicl.el
  (defvar context-atomicl
    (make-mu4e-context
     :name "Atomicl"
     :match-func
     (lambda (msg)
       (when msg
         (string-prefix-p "/atomicl.net/primary/vselvarajah@atomicl.net" (mu4e-message-field msg :maildir))))
     :vars '((user-mail-address   . "vselvarajah@atomicl.net")
             (mu4e-compose-signature . "Vithurshan SELVARAJAH\n* Mon site web – www.atomicl.net\n* LinkedIn – www.linkedin.com/in/vithurshan-selvarajah\n* RSS Feed - www.atomicl.net/index.xml")
             ;; The SMTP password will be retrieved from the file
             ;; defined in the "auth-sources" variable
             (smtpmail-smtp-server  . "noradrenaline.atomicl.net")
             (smtpmail-smtp-service . 587)
  	   (smtpmail-stream-type  . starttls)
             (mu4e-drafts-folder  . "/atomicl.net/primary/vselvarajah@atomicl.net/Drafts")
             (mu4e-sent-folder    . "/atomicl.net/primary/vselvarajah@atomicl.net/Sent")
             (mu4e-trash-folder   . "/atomicl.net/primary/vselvarajah@atomicl.net/Trash")
             (mu4e-maildir-shortcuts . (("/atomicl.net/primary/vselvarajah@atomicl.net/Inbox"         . ?i)
                                        ("/atomicl.net/primary/vselvarajah@atomicl.net/Sent"          . ?s)
                                        ("/atomicl.net/primary/vselvarajah@atomicl.net/Trash"         . ?t)
                                        ("/atomicl.net/primary/vselvarajah@atomicl.net/Drafts"         . ?d)))
             )))
#+end_src
...
#+begin_src emacs-lisp :results none
  ...
  (use-package mu4e
    ...
    ;; I donn't want to sync remote maildir because it's delegate mbsync
    ...
    (load "~/.config/emacs/mails/mu4e-context-atomicl.el")
    ...
    ;; Bookmarks
    (setq mu4e-bookmarks
    	'((:name "All Inboxes" :query "maildir:/vithurshan@laposte.net/Inbox OR m:/vithurshanselvarajah@gmail.com/Inbox OR m:/atomicl.net/primary/vselvarajah@atomicl.net/Inbox OR m:/atomicl.net/secondary/vselvarajah@atomicl.net/Inbox" :key ?i)
            (:name "Sent Items" :query "m:/vithurshan@laposte.net/Sent\\ Items OR m:/vithurshanselvarajah@gmail.com/[Gmail]/Messages\\ envoy&AOk-s OR m:/atomicl.net/primary/vselvarajah@atomicl.net/Sent" :key ?s)
            (:name "Deleted Mails" :query "m:/vithurshan@laposte.net/Deleted\\ Items OR m:/vithurshanselvarajah@gmail.com/[Gmail]/Corbeille OR m:/atomicl.net/primary/vselvarajah@atomicl.net/Trash" :key ?t)
            (:name "Draft Mails" :query "m:/vithurshan@laposte.net/DRAFT OR m:/vithurshanselvarajah@gmail.com/[Gmail]/Brouillons OR m:/atomicl.net/primary/vselvarajah@atomicl.net/Drafts" :key ?d)
            (:name "Today's messages" :query "(m:/vithurshan@laposte.net/Inbox OR m:/vithurshanselvarajah@gmail.com/Inbox OR OR m:/atomicl.net/primary/vselvarajah@atomicl.net/Inbox) AND date:today..now" :key ?n)
            (:name "Last 7 days" :query "(m:/vithurshan@laposte.net/Inbox OR m:/vithurshanselvarajah@gmail.com/Inbox OR OR m:/atomicl.net/primary/vselvarajah@atomicl.net/Inbox) AND date:7d..now" :hide-unread t :key ?w)
            (:name "Messages with images" :query "mime:image/*" :key ?p)))


    ;; Keybinding for saving attachments in message view
    (with-eval-after-load 'mu4e-view
      (define-key mu4e-view-mode-map (kbd "C-c e") #'vts/save-all-attachments))
