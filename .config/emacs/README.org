...
- [[#hold-mail][HOLD Mail]]
  - [[#mu4e][Mu4e]]
- [[#ongoing-run-shell-command][ONGOING Run Shell Command]]
...
#+begin_src emacs-lisp :results none :tangle no
  (defvar *afilename-cmd*
    '(("/home/vts/git/cv/cv.tex" . "pdflatex ~/git/cv/cv.tex")
...
** TODO Syncing Mails
:LOGBOOK:
- State "TODO"       from              [2023-11-08 mer. 20:53] \\
  Move useful info to isync/readme.org and delete this section.
  Then make a link to isync/readme.org file
:END:
...
#+begin_src emacs-lisp :results none :tangle yes
  ...
  (use-package mu4e
    ...
    ;; This is set to 't' to avoid mail syncing issues when using mbsync
    (setq mu4e-change-filenames-when-moving t)
    ;; (setq mu4e-user-mail-address-list '("vithurshanselvarajah@gmail.com"
    ...
    ;; Refresh mail using isync every 10 minutes
    (setq mu4e-update-interval (* 10 60))
    (setq mu4e-get-mail-command "/usr/bin/mbsync -Va -c /home/vts/git/dotrice/.config/isync/mbsyncrc")
          ...
          (list
           ...
           (make-mu4e-context
            ...
            :vars '((user-mail-address   . "vithurshan@laposte.net")
                    (user-full-name      . "Vithurshan SELVARAJAH Laposte")
           ...
           (make-mu4e-context
            ...
            :vars '((user-mail-address . "vithurshanselvarajah@gmail.com")
                    (user-full-name    . "Vithurshan SELVARAJAH Gmail")
